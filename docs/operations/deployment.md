...
[ADR-0027](../adr/0027-private-s3-visit-photo-storage.md), 인프라 생성·변경 절차는
...
- ECS task는 SSM Parameter Store에서 DB 연결 설정, TourAPI 인증키와 Groq API 키를
  주입받고 Task Role로 방문 사진 버킷 `visits/*`만 읽고 쓴다. 정적 AWS 키는 주입하지
  않는다.
...
실행 직전에 다시 확인한다. 실행 후 service stability와 Health를 다시 확인한다. 방문
사진 버킷과 기존 객체는 애플리케이션 롤백 중 삭제하지 않는다.

## Terraform 변경

Terraform은 자동 배포 workflow에서 `apply`하지 않는다. `terraform plan` 결과를 사람이
검토한 뒤 별도 승인으로 `apply`하며, `destroy`, RDS 삭제 보호 해제와 데이터 변경은
이 자동 애플리케이션 배포 절차에 포함하지 않는다. 방문 사진 버킷 제거는 객체 보존·복구
여부를 먼저 결정하고 별도 승인한 경우에만 수행한다.

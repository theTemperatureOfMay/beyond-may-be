...
- 변경한 대상: 성능 전용 Docker Compose, k6 회원가입 smoke·load·stress·spike 시나리오,
  PowerShell 실행기와 행동 시험, Spring `performance` 프로필과 Prometheus registry,
...
- 미해결: 같은 머신에서 load 3회 중앙값 기준선을 만드는 절차와 결과는 #29에서 다룬다.
...
- 고정 k6 이미지의 `inspect`로 smoke 30초, load 10분, stress 15분, spike 3분,
  ...
  Compose 모델 검사도 통과했다.
- 실제 Docker에서 load·stress·spike를 실행해 프로필별 결과 산출물과 성공·실패 수명주기를
  확인했다. load는 누락 요청이 발생해 엄격한 유효성 기준을 통과하지 못했고, 기준선으로
  사용하지 않는다. stress와 spike는 독립 실행과 성공 후 앱·성능 DB 정리를 확인했다.

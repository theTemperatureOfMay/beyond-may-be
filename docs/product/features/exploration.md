...
코스 장소와 일정은 공유 기간에는 공개 `GET /api/v1/courses/{courseId}`로 조회한다.
공유 기간이 끝난 뒤에는 인증된 현재 또는 과거 참여자만 같은 API로 계속 조회할 수 있고,
비인증 사용자와 참여 이력이 없는 사용자는 410을 받는다. 인증된 현재 또는 과거 참여자는
`GET /api/v1/explorations/{explorationId}`에서 탐험 상태, 팀 인원·방문 수, 코스 진행률과
현재 참여자의 실행 권한을 함께 조회한다.

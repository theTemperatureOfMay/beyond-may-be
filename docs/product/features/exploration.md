...
코스 ID만 알고 있는 인증된 현재 또는 과거 참여자는
`GET /api/v1/courses/{courseId}/exploration`으로 연결된 탐험 ID를 조회한다.
응답은 공통 `data` 안의 `explorationId` 하나이며, 탐험·참여 상태와 공유 링크 만료에
관계없이 조회할 수 있다. 미인증은 401, 해당 탐험의 참여 기록이 없으면 403,
연결된 탐험이 없으면(없는 코스·미확정 코스 포함) 404를 반환한다.
...
- 최초 동의 모달은 탐험 상태 조회가 끝난 뒤 `ONGOING` 탐험의 `ACTIVE` 참여자가 공유하지 않는 경우에만 표시한다.
- '나중에' 또는 닫기를 선택하면 공유를 켜지 않고, 해당 탐험·참여자의 팝업 확인 여부만 이 브라우저에 저장하여 재진입·새로고침 시 다시 묻지 않는다. 다른 탐험·참여자는 별도로 안내하며 브라우저 저장소를 삭제하거나 다른 브라우저를 사용하면 다시 표시될 수 있다.
- 사이드바 → 설정에서 현재 탐험의 위치 공유를 켜거나 끌 수 있다. 진행 중인 탐험이 없으면 토글은 비활성 상태로 표시하고 클릭·키보드 활성화 시 안내만 제공한다. 저장 성공 시 팝업 확인 여부도 기록한다. 서버 동의 상태가 정본이며 기기의 위치 권한과는 별개다.
- 설정에는 별도의 GPS 사용·위치 권한 토글을 제공하지 않는다. 실제 위치 취득은 브라우저·기기의 권한에 따르며, 이전 앱 GPS 설정값은 위치 요청에 사용하지 않는다. 좌표는 영속 저장하지 않고 별도 위치 체험 모드는 유지한다.
...
- 상세 시트 우측 상단에 닫기(X) 버튼을 제공하며 내용을 스크롤해도 버튼 위치를 유지한다. 배경 지도는 흐림 효과 없이 반투명 검정 오버레이(25%) 한 겹으로 살짝 어둡게 표시한다.
- 주변 추천에서 상세를 열면 추천 목록과 그 오버레이는 숨기고, 상세를 닫으면 기존 추천 목록으로 돌아간다.
...
4.4.2도 인증이 필요 없는 `GET /api/v1/places/{placeId}`를 사용한다. 저장된 장소 상세를

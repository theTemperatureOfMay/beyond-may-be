...
- 설정의 GPS 위치 사용은 브라우저별 앱 설정으로 보관하며 기본값은 켜짐이다. 끄면 실제 GPS 추적·코스 시작 시 위치 요청을 중단하고 보관 중인 좌표를 비운다. 좌표는 영속 저장하지 않는다. 탐험 없이도 변경할 수 있으며 브라우저 권한과 서버의 팀원 공유 동의는 변경하지 않는다. 별도 위치 체험 모드는 실제 GPS 설정과 구분한다.

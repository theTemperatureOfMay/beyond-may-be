...
| 5.2.1 | 방문한 장소 목록 조회 | 코스·주변 장소를 포함한 팀·개인 방문 기록과 선택적 다중 사진 | 데모 핵심 | `부분 구현` | `POST /api/v1/visits`가 코스·주변 Place 기반 개인 방문을 저장하고, Visit 작성자인 `ACTIVE Participant`는 `POST /api/v1/visits/{visitId}/photos`로 10MB 이하 JPEG·PNG·WebP를 비공개 S3에 제한 없이 한 장씩 첨부한다. 응답은 1시간 presigned GET URL을 제공한다. 팀 방문 목록·재접속 복구 GET은 아직 없다(ADR-0027). |
...
- 실제 기능 API는 회원가입·로그인, 추천·회차 반응, AI 코스 생성, 코스 확정,
  ...
  비공개 S3와 ECS Task Role로 구현했다. OWNER 조기 완료와 방문 목록 조회, 위치 replay와
  외부 broker는 아직 없다(ADR-0022~0027).

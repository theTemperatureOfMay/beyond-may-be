...
| 3.1.1 | 추천 코스 지도 보기 | 코스 장소·순서·카카오맵 도보·대중교통 동선·요약 조회 | 데모 핵심 | `부분 구현` | `GET /api/v1/routes`가 카카오맵 REST API의 도보·대중교통 endpoint를 호출하고, 대중교통 탑승 전·하차 후 도보 구간을 함께 조합한다. 코스 조회 API 응답과 프런트 지도 연동은 별도 작업이다. |

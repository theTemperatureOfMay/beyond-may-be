...
- 현재 상태: 구현 완료 9개, 부분 구현 6개, 미구현 23개, 명세 불일치 1개, 동작 2개,
  대상 아님 8개
- `동작`으로 판정된 기능: 2.1.1 여행 기간 설정, 6.3.1 공통 빈 상태
- `데모 검증`으로 판정된 기능: 없음
...
| 2.1.2 | AI 추천 장소 목록 조회 | 4개 성향 비율·기간 기반으로 DB 장소 20곳을 유형별 배분하고 최소 선택 수 충족까지 중복 없이 추가 제공 | 데모 핵심 | `부분 구현` | 같은 API가 활성 DB 장소를 최대 나머지 방식으로 배분·재배분하고 유형별 최대 3배·전체 최대 60곳을 Groq `openai/gpt-oss-20b`가 선별하게 한다. strict ID 응답을 저장 직전 전부 검증하며 외부·계약 실패는 안정적인 규칙 결과 전체로 대체한다. 최초 유형별 할당량이 부족하면 TourAPI 광주 변경분을 최대 한 번 저장·재사용하고, 새 추천 응답의 빈 설명·운영시간은 응답과 분리된 `detailCommon2`·`detailIntro2` 최선 노력 작업으로 조건부 보강한다. 추가 회차 반응 저장은 아직 없다. |
...
| 6.3.1 | 공통 빈 상태 | 정상 0건 응답과 기능별 대체 경로 계약 | 일반 | `동작` | 추천 생성 API가 활성 후보 0건을 오류가 아닌 `200 OK`와 빈 `places`로 반환하며 서비스 테스트가 계약을 검증한다. |

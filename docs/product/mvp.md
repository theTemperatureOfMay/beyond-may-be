...
| 6.1.3 | 요청 지연 (타임아웃) | API·외부 연동 timeout 설정과 오류 응답 | 일반 | `부분 구현` | 추천 생성의 TourAPI 연결·응답 timeout은 각각 최대 5초, Groq 연결·응답 timeout은 최대 20초이며 둘 다 자동 재시도 없이 규칙 경로로 복구한다. 전체 HTTP 30초 제한과 다른 외부 연동의 timeout 계약은 아직 구현되지 않았다. |

...
- 현재 상태: 구현 완료 8개, 부분 구현 6개, 미구현 26개, 명세 불일치 1개, 대상 아님 8개
...
| 6.1.5 | 세션 만료 | 세션 토큰 검증과 401 응답 | 일반 | `부분 구현` | `AuthTokenService`가 30일 만료 토큰을 발급하고 만료·무효 토큰을 거부하며 `TokenAuthenticationFilter`가 이를 인증에 사용한다. 다만 인증 실패 응답은 현재 403이고 명세의 401 계약과 프런트엔드 재로그인 흐름은 아직 충족하지 않는다. |

...
- 현재 상태: 구현 완료 9개, 부분 구현 12개, 미구현 9개, 명세 불일치 1개, 동작 12개,
  대상 아님 6개
- `동작`으로 판정된 기능: 2.1.1 여행 기간 설정, 2.1.2 AI 추천 장소 목록 조회,
  ...
  4.3.1 현재 위치 표시, 4.3.3 지도 밝히기, 4.4.1 주변 장소 추천,
  6.3.1 공통 빈 상태
- `데모 검증`으로 판정된 기능: 없음
...
| 4.2.1 | 코스 미리보기 | 확정 코스·장소·동선·요약 조회 | 데모 핵심 | `구현 완료` | `GET /api/v1/courses/{courseId}`(`CourseService.getCourseDetail`)가 확정 코스 정보와 일자·순서대로 정렬된 장소 목록(좌표 포함)을 반환한다. 인증 불필요(비로그인 미리보기 허용), 공유 만료 시 410. 인증된 현재·과거 참여자는 `GET /api/v1/courses/{courseId}/exploration`으로 탐험 ID를 조회하고 `GET /api/v1/explorations/{explorationId}`로 탐험 상태와 실행 권한을 함께 조회한다. 동선 폴리라인은 프런트가 Kakao Maps/TMAP으로 렌더링하므로 서버 응답에 없다(ADR-0009). |
...
| 4.3.2 | 팀원 진행 상태 보기 | 방문 수 조회, 실시간 갱신과 참여자별 위치 공유 옵트인 저장 | 데모 핵심 | `부분 구현` | 팀원 목록 API가 참여자별 고유 방문 수와 위치 공유 동의 상태를 반환한다. 저빈도 상태는 `/events`, 개인 방문과 팀 진행률은 `/visits`, 옵트인 위치는 휘발 `/locations` JSON envelope로 분리하며 참여자 범위로 인가한다. 위치는 userId 없이 전파하고 replay하지 않으며 공유 해제 상태 이벤트로 마커를 제거한다. 탐험·참여자와 팀 방문 기록 HTTP 조회로 상태·집계·개별 방문을 복구하지만 마지막 위치 replay는 없다(ADR-0024~0027). |
...
| 4.4.2 | 장소 상세 보기 | 장소 상세 조회와 코스 미포함 주변 장소 방문 인증 연결 | 일반 | `부분 구현` | 인증된 장소 상세 GET과 코스 미포함 Place 방문 저장·실시간 전파는 동작하며 주변 방문은 코스 진행률에서 제외한다. 현재·과거 참여자는 팀 방문 기록 GET으로 장소별 방문 여부와 인증 참여자를 복구할 수 있다. |
...
- 2026-09-03 팀 방문 기록 GET의 현재·과거 참여자 인가, 전체 팀·주변 장소 포함,
  ...
  통과했고 `spotlessCheck`, 전체 `build`, Postman Collection·saved body JSON 파싱,
  제품 지식·하네스 semantic 검사와 `git diff --check`가 통과했다. 실제 AWS 서명 URL
  조회와 프런트엔드 연동은 확인하지 않았다.
- 2026-09-03 방문 사진 첨부 API의 작성자·`ACTIVE Participant` 인가, 파일 형식·10MB
  검증, Visit 잠금 기반 표시 순서, S3 업로드·DB 실패 보상과 1시간 presigned GET URL을
  ADR-0027·기능 명세·코드·테스트·Terraform과 대조했다. 전체 393개 테스트가 실패·오류·
  건너뜀 없이 통과했고 `spotlessCheck`, 전체 `build`, Terraform 정적 검증, Postman JSON
  파싱, 제품 지식·하네스 semantic 검사와 `git diff --check`가 통과했다. 실제 AWS 적용·
  업로드와 프런트엔드 multipart·URL 재발급 연동은 확인하지 않았다.

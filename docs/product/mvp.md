...
| 2.1.2 | AI 추천 장소 목록 조회 | 4개 성향 비율·기간 기반으로 DB 장소를 회차당 최대 15곳 배분하고 최소 선택 수 충족까지 추가 제공 | 데모 핵심 | `동작` | `POST /api/v1/recommendations/sets`가 최초 회차를 저장하고 `GET /api/v1/recommendations`가 같은 사용자의 저장 회차·확정 반응을 복구한다. 추가 회차는 미노출 활성 장소와 TourAPI 보충만 사용하며, 마지막 1~14곳은 중복 없는 최종 부분 회차로 저장한다. |

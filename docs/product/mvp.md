...
| 2.1.2 | AI 추천 장소 목록 조회 | 4개 성향 비율·기간 기반으로 DB 장소 20곳을 유형별 배분하고 최소 선택 수 충족까지 추가 제공 | 데모 핵심 | `동작` | `POST /api/v1/recommendations/sets`가 최초 회차를 저장하고, 회차 반응 API가 미노출 활성 장소와 TourAPI 보충을 우선해 정확히 20곳을 누적한다. 그래도 부족한 마지막 경우에는 기존 싫어요 장소를 ID당 한 번 재추천하며, 부분 추가 회차는 저장하지 않는다. |
...
| 2.2.2 | 장소 스와이프 — 싫어요 | 추천 회차 종료 후 좋아요·싫어요 일괄 저장과 추가 회차 생성 | 데모 핵심 | `동작` | 회차 ID의 정확한 분할을 검증하고 싫어요를 저장한다. 추가 회차 후보가 부족하면 TourAPI 보충 뒤 싫어요를 마지막 보충 풀로 한 번만 사용하며, 새 반응이 이전 상태를 대체한다. |

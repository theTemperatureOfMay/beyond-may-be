...
| 6.1.3 | 요청 지연 (타임아웃) | API·외부 연동 timeout 설정과 오류 응답 | 일반 | `부분 구현` | 추천 생성은 전체 30초를 넘으면 `RECOMMENDATION503`으로 종료하고 자동 재시도하지 않는다. TourAPI 최대 5초와 Groq 최대 20초 timeout은 각각 기존 DB 후보·규칙 경로로 복구한다. 다른 외부 연동의 timeout 계약은 아직 구현되지 않았다. |
...
- 실제 기능 API는 회원가입·로그인, TourAPI 부족분 보충과 Groq AI 선별·규칙 폴백을
  포함한 추천 세트 생성, 코스 확정, 팀 합류·팀원 목록·탐험 시작, 방문 인증과
  Socket.IO 실시간 채널이다(ADR-0012, ADR-0014, ADR-0015). AI 코스 생성은 아직 없다.

...
- 기준 일자: 2026-09-19 (1.1.1 홈 복구 계약 갱신)
- 제품 범위: 상세 기능 명세의 49개 소기능 전체
- 현재 상태: 구현 완료 9개, 부분 구현 12개, 미구현 8개, 명세 불일치 1개, 동작 13개,
  대상 아님 6개
- `동작`으로 판정된 기능: 1.1.1 서비스 시작 화면 진입, 2.1.1 여행 기간 설정, 2.1.2 AI 추천 장소 목록 조회,
...
|---|---|---|---|---|---|
| 1.1.1 | 서비스 시작 화면 진입 | 본인 생성 코스와 ACTIVE 참여 탐험 조회 | 데모 핵심 | `동작` | GET /api/v1/courses와 BEFORE·ONGOING 탐험 목록으로 복구한다. 생성자 격리·수정 시각 정렬·LEFT 제외·인증과 서비스/저장소 테스트로 검증한다. 화면 정책은 온보딩 명세를 따른다. |
...
| ID | 소기능 | 백엔드 책임 | 우선순위 | 상태 | 근거·비고 |
|---|---|---|---|---|---|
| 2.1.1 | 여행 기간 설정 | 기간·날짜 검증과 최소 장소 수 산정 | 데모 핵심 | `동작` | `POST /api/v1/recommendations/sets`가 네 일정 유형의 날짜 규칙과 과거일을 검증하고 최소 선택 수 3·5·7·15를 반환하며 서비스·MockMvc 테스트가 통과한다. |
| 2.1.2 | AI 추천 장소 목록 조회 | 4개 성향 비율·기간 기반으로 DB 장소를 회차당 최대 15곳 배분하고 최소 선택 수 충족까지 추가 제공 | 데모 핵심 | `동작` | `POST /api/v1/recommendations/sets`가 일정과 관계없이 현재 추천·반응 진행을 새 결과로 생성·교체하고, `GET /api/v1/recommendations`가 같은 사용자의 저장 회차·확정 반응을 복구한다. 추가 회차는 미노출 활성 장소와 TourAPI 보충만 사용하며, 마지막 1~14곳은 중복 없는 최종 부분 회차로 저장한다. |
...
| 2.2.4 | 장소 상세 보기 | 주소·운영시간·일반 설명·5·18 검수 문구 조회 | 일반 | `동작` | 비인증 `GET /api/v1/places/{placeId}`가 활성 장소 카탈로그를 반환한다. 비어 있는 설명·운영시간은 TourAPI로 동기 보강해 저장·즉시 반환하고, 정상 빈값은 정보 없음 문구로 확정하며 외부 오류는 `PLACE503`으로 재시도 가능하게 둔다. |
| 2.3.1 | 선택 장소 목록 확인 | 기간별 최소 장소 수 검증 | 데모 핵심 | `동작` | 회차 반응 API가 전체 저장 좋아요 수와 기간별 최소 수 3·5·7·15를 비교해 선택 수와 준비 여부를 반환하며, 이전 회차 선택 삭제도 재전송으로 반영한다. |
...
| ID | 소기능 | 백엔드 책임 | 우선순위 | 상태 | 근거·비고 |
|---|---|---|---|---|---|
| 3.1.0 | AI 코스 생성 | 선택 장소의 방문 순서와 코스 정보 생성·저장 | 데모 핵심 | `동작` | `CourseService.generate`가 선택 장소·Haversine 거리·운영시간을 Groq strict JSON Schema로 전달해 일자별 순서를 받고, 정확한 ID·일수·균형을 검증한다. 실패하면 최근접 거리와 FOODIE 점심·저녁 슬롯 fallback을 적용하며, 07:00 시작·60분 체류의 `Course(DRAFT)`·`CoursePlace`를 원자 저장한다. API는 60초 제한과 `COURSE503_2` timeout 계약을 가진다. 관련 서비스·클라이언트·MockMvc 테스트가 통과한다. 지도와 경로는 Kakao Maps API를 사용한다. |
| 3.1.1 | 추천 코스 지도 보기 | 코스 장소·순서·카카오맵 도보·대중교통 동선·요약 조회 | 데모 핵심 | `부분 구현` | `GET /api/v1/routes`가 카카오맵 REST API의 도보·대중교통 endpoint를 호출하고, 대중교통 탑승 전·하차 후 도보 구간을 함께 조합한다. 보조 도보 조회가 실패해도 `ROUTE200_1` 안내와 함께 대중교통 경로를 반환한다. 코스 조회 API 응답과 프런트 지도 연동은 별도 작업이다. |
...
| 3.3.1 | 코스 확정 | AI 생성 성공 시 draft·courseId 저장, confirmed 전환, 수정·재설계 진입 차단 | 데모 핵심 | `부분 구현` | `POST /api/v1/courses/{courseId}/confirm`이 소유자 검증 후 사용자 행을 잠그고 다른 `BEFORE`·`ONGOING` 활성 참여가 없을 때만 `DRAFT→CONFIRMED` 전환, `Exploration(BEFORE)`·owner Participant 생성과 공유 만료(3일) 설정을 수행한다(`CourseService.confirm`). 활성 참여가 있으면 `409 EXPLORATION409`를 반환한다. AI 수정 횟수 제한은 아직 없다. 확정 후 프런트엔드는 코스 수정·재설계 진입을 제공하지 않는다. |
...
| 4.2.1 | 코스 미리보기 | 확정 코스·장소·동선·요약 조회 | 데모 핵심 | `구현 완료` | `GET /api/v1/courses/{courseId}`(`CourseService.getCourseDetail`)가 확정 코스 정보와 일자·순서대로 정렬된 장소 목록(좌표 포함)을 반환한다. 인증 불필요(비로그인 미리보기 허용), 공유 만료 시 410. 인증된 현재·과거 참여자는 `GET /api/v1/courses/{courseId}/exploration`으로 탐험 ID를 조회하고 `GET /api/v1/explorations/{explorationId}`로 탐험 상태와 실행 권한을 함께 조회한다. 경로는 `GET /api/v1/routes`와 Kakao Maps를 사용하며 경로 결과는 저장하지 않는다(ADR-0029). |
...
| 4.4.2 | 장소 상세 보기 | 장소 상세 조회와 코스 미포함 주변 장소 방문 인증 연결 | 일반 | `부분 구현` | 비인증 장소 상세 GET과 코스 미포함 Place 방문 저장·실시간 전파는 동작하며 주변 방문은 코스 진행률에서 제외한다. 현재·과거 참여자는 팀 방문 기록 GET으로 장소별 방문 여부와 인증 참여자를 복구할 수 있다. |

...
탐험 중 다른 공유 링크를 열어 `EXPLORATION409`를 받으면 `data.activeExplorationId`로
기존 지도로 이동할 수 있다. 나가고 새로 참여하기를 선택하면 확인 후 기존 탐험의
`POST /api/v1/explorations/{explorationId}/leave`가 성공한 다음 새 코스의 join을 호출한다.
OWNER 이탈 시 남은 활성 팀원에게 탐험 소유권을 이전하며 원래 코스 작성자는 유지한다.
참여·방문·사진은 보존하고, 새 합류가 실패해도 이탈은 자동 취소하지 않는다.
...
추천은 회차당 15곳씩 제공하며 네 유형을 2곳씩 먼저 보장한 뒤 남은 7곳을 사용자의
점수 비율에 맞춰 유형별 장소 수를 배분한다. 예를 들어 점수가 4:3:2:1이면 각각
5곳·4곳·3곳·3곳을 제공한다. 최초 배분에 필요한 DB 장소가 부족하면 서버는 TourAPI 광주 변경분을 한 번
조회해 유효한 신규 장소를 저장하고 같은 추천 후보로 사용한다. 외부 조회가 실패하거나
새 장소가 없으면 기존 DB 후보를 재배분한다. 기간별 최소 장소 수를 충족하지 못했더라도
최초 추천이 빈 목록이면 빈 상태로 끝낸다. 그 밖에는 미노출 활성 장소를 최대 15곳 추가로
제공한다. 미노출 후보가 부족하면 해당 추가 회차 요청에서 TourAPI 변경분을 한 번 조회하고,
그래도 15곳보다 적으면 남은 장소를 중복 없이 최종 부분 회차로 제공한다. 각 회차가
끝나면 좋아요·싫어요 전체를 서버에 저장한다. 재진입 시 서버는 저장된 모든 회차와 확정
반응을 반환하며, 아직 일괄 전송하지 않은 반응과 현재 카드 위치는 프런트엔드가 관리한다.
서버가 확정한 회차별
최대 45곳 안에서 Groq가 다음 ID 순서를 선별한다.
...
방문 인증 후에는 해당 방문에 사진 최대 3장과 선택적 메모를 저장한다. 건너뛰면 인증 이력만 유지한다. 작성자는 탐험 완료 후에도 기록을 저장할 수 있으며 이탈한 참여자는 저장할 수 없다.
...
| 활성 탐험 진행 중 | 팀 탐험 지도 |
| 활성 탐험 시작 전 | 홈에서 기존 코스 이동 팝업. 네: 탐험 시작 화면, 아니요: 홈 유지 |
| 활성 탐험 없이 초안 있음 | 최신 수정 초안 이어 만들기 또는 날짜 선택 |
| 활성 탐험·초안 없음, 완료 탐험만 있음 | 날짜 선택부터 새 코스 생성 |
| 코스·탐험 조회 실패 | 홈에서 오류 표시 및 재시도 |

순서와 조회 계약은 [홈 진입 정책](features/onboarding-preference.md#111-서비스-시작-화면-진입)을 따른다.

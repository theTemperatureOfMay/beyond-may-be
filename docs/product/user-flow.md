...
새 장소가 없으면 기존 DB 후보를 재배분한다. 기간별 최소 장소 수를 충족하지 못하면 새
20곳을 추가로 제공한다. 서버가 확정한 최대 60곳 안에서 Groq가 첫 회차의 ID 순서를
선별하며, 응답 전체 검증이나 외부 호출이 실패하면 같은 입력의 안정적인 규칙 순서를
사용한다. 새 추천 응답에 포함된 장소의 설명·운영시간이 비어 있으면 응답과 분리된
최선 노력 작업으로 DB를 보강한다. 최소 기준을 만족하면 선택한 장소를 코스 생성 단계로
전달한다.
...
인증 요청은 REST API로 처리한다. 방문 인증은 서버가 100m 반경과 50m 위치 정확도

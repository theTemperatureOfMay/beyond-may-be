...
|---|---|---|---|
| 관광 장소·분류 데이터 수집 | 장소 데이터 최초 수집 | `KorService2/ldongCode2`, `KorService2/lclsSystmCode2`, `KorService2/areaBasedList2` | 광주 장소와 TourAPI 신분류체계를 저장하고 여행 성향별 장소 풀 구성 |
| 중심관광지 순위 수집 | 중심관광지 순위 갱신 | `LocgoHubTarService1/areaBasedList1` | 추천 후보의 보조 순위 데이터 구성 |
| 2.1.2 AI 추천 장소 목록 조회 | 사용자 추천 요청 중 최초 유형별 할당량이 부족할 때 | `KorService2/areaBasedSyncList2` 최대 1회 | 유효한 광주 변경분을 DB에 먼저 저장하고 같은 요청 후보로 포함한 뒤 최대 15곳 구성 |
| 2.2.2 장소 스와이프 — 싫어요 | 추가 회차의 미노출 활성 후보가 15곳보다 적을 때 | `KorService2/areaBasedSyncList2` 최대 1회 | 유효한 신규 장소를 DB에 저장하고 다시 계산한 뒤 남은 미노출 장소를 최대 15곳의 최종 회차로 구성 |
...
| 2.2.4 장소 상세 보기 | 사용자가 추천 장소 상세를 열었고 저장된 설명·운영시간이 비어 있을 때 | `KorService2/detailCommon2`, `KorService2/detailIntro2` 중 비어 있는 필드에 필요한 API | 보강 값을 저장해 같은 상세 응답에 포함하고, 정상 빈값은 정보 없음 문구로 확정 |
| 4.4.1 주변 장소 추천 | 사용자가 주변 장소 더보기 클릭 | `KorService2/locationBasedList2` | 현재 GPS에서 1km 이내 관광 장소 후보 조회 |
| 4.4.2 탐험 지도 내 장소 상세 | 사용자가 탐험 장소 상세를 열었고 저장된 설명·운영시간이 비어 있을 때 | 2.2.4와 같은 `detailCommon2`, `detailIntro2` | 2.2.4와 같은 저장 우선·동기 보강 상세 사용 |

추천 요청의 부족분 보충, 상세정보 사후·상세 GET 동기 보강과 외부 실패 대체는
[장소 선택 기능 명세](features/place-selection.md)를 따른다. 같은 명세가 Groq
`openai/gpt-oss-20b`의 최대 45곳 후보 선별, strict ID 검증과 규칙 기반 전체 폴백

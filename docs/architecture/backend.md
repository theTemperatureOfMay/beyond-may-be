...
- 회차당 장소 20곳을 제공하고, 회차가 끝날 때 프런트엔드가 해당 회차의
  좋아요·싫어요를 한 번에 전송한다.
- 회차 종료 시 최소 선택 수를 충족하지 못하면 미노출 활성 장소를 우선한 다음 20곳을
  같은 배열 뒤에 붙인다. 최초 회차는 후보 부족으로 20곳 미만일 수 있지만 추가 회차는
  20곳일 때만 저장한다.
- 최초 회차가 0곳이면 빈 상태로 유지하며 빈 반응 요청으로 외부 보충이나 추가 회차 생성을
  다시 시작하지 않는다.
- 미노출 후보와 TourAPI 보충으로도 20곳을 만들 수 없으면 현재 좋아요가 아닌 기존
  싫어요 장소를 ID당 한 번만 재추천해 부족분을 채운다. 재추천 반응은 해당 장소의 이전
  최종 반응을 대체하며, 합계가 20곳보다 적으면 추가 회차를 만들지 않는다.
...
- 전체 좋아요가 최소 수보다 적으면 미완료인 기존 다음 회차를 반환하거나 미노출 장소와
  필요한 싫어요 재추천 장소로 만든 새 회차를 배열 뒤에 붙인다. 같은 이전 회차 요청은
  이미 생성된 재추천 회차를 중복 생성하지 않고 그대로 반환한다. Groq 호출 전후의 추천
...
상태 소유권과 추가 회차 부족 처리 규칙은
[ADR-0017](../adr/0017-recommendation-batch-shortage-fallback.md)을 따른다.

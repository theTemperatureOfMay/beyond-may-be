...
| 작업 | 권한 |
|---|---|
| 질문·성향 계산, 유효한 공유 코스 미리보기, MBTI 유형별 장소 추천, 장소 상세 조회 | 비인증 허용 |
...
- 회차당 장소 15곳을 제공하고, 회차가 끝날 때 프런트엔드가 해당 회차의
  좋아요·싫어요를 한 번에 전송한다.
- 회차 종료 시 최소 선택 수를 충족하지 못하면 미노출 활성 장소를 최대 15곳까지 같은
  배열 뒤에 붙인다. 마지막 1~14곳의 부분 회차는 해당 추천 세트의 최종 회차다.
- 최초 회차가 0곳이면 빈 상태로 유지하며 빈 반응 요청으로 외부 보충이나 추가 회차 생성을
  다시 시작하지 않는다.
- 미노출 후보와 TourAPI 보충 뒤 1~14곳이 남으면 모두 최종 부분 회차로 저장하고, 0곳이면
...
- DB 규칙 기반 생성은 활성 장소 최대 15곳을 유형별 2곳씩 먼저 배정한 뒤 남은 7곳을
  최대 나머지 방식으로 배분하고, 부족 유형은 원래 성향 점수 비율로 여유 유형에
  재배분한다. 사용자·일정·날짜가 같으면 안정적인 셔플 결과도 같다.
- AI 후보는 유형별 최종 할당량의 최대 3배, 전체 최대 45곳이며 Groq
  ...
  대체한다.
- 최초 유형별 할당량이 부족하거나 추가 회차의 미노출 후보가 15곳보다 적으면
...
- 회차 반응도 사용자 행 잠금으로 직렬화한다. 다음 회차가 필요하면 첫 번째 짧은
  ...
  확정한다.
- 새로고침이나 이탈 뒤 재진입은 현재 추천 조회 API로 저장된 결과를 복구하며, 저장된 ID의
  장소가 삭제됐거나 비활성이면 응답에서만 제외한다. 추천 생성 API는 일정이 같아도 현재
  추천 세트와 반응을 새 결과로 교체한다. 회차 반응 검증은 원래 저장된 전체 ID 또는 현재 응답에

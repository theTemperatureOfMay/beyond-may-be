...
| 만료된 공유 코스 미리보기 | 해당 `Exploration`의 현재 또는 과거 `Participant` |
...
- 공유 링크 만료는 `share_expires_at`으로 판정한다. 만료된 코스 미리보기는 인증된 현재
  또는 과거 Participant만 허용하고 그 외 조회와 신규 합류 요청은 410으로 거부한다.

...
3. plan이면 상태가 `승인됨`인지 확인한다. GitHub ticket이면 구현 직전에 현재 body, comments와
   완료 조건을 다시 읽는다.
4. 승인된 plan은 별도 구현 요청이나 두 번째 승인 없이 즉시 실행 입력으로 사용한다.
5. 사용자가 특정 implementation ticket의 구현을 명확히 요청하면 승인된 입력으로 본다.
   미해결 blocker가 있으면 구현하지 않고 현재 blocker를 보고한다.
6. spec만 지정되면 구현하지 않고 `/to-tickets`로 넘긴다. 읽기·분류·초안 작성은 자동으로
   시작할 수 있지만 GitHub 게시는 최종 대상과 내용을 보여 주고 별도 승인받는다.
7. 승인된 실행 입력이 없으면 구현하지 않고 필요한 입력과 승인을 요청한다.

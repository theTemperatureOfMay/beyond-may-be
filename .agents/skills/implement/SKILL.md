...
Local Markdown ticket은 각 acceptance criterion의 충족·검증 증거가 있을 때만 해당 체크박스를
`[x]`로 갱신하고 criterion 문구는 구현 중 수정하지 않는다. 모든 체크박스가 `[x]`이고 전체 검증이
성공한 경우에만 `Status: resolved`로 갱신한다. 실패하거나 미실행 검사가 있으면 해결 상태로 바꾸지 않는다.
연결된 `spec.md`는 수정하지 않는다. 중단되면 구현 기록에 완료한 항목, `구현 실패`와 이유를 남긴다.

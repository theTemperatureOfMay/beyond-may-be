...
function Invoke-Runner {
    ...
    [System.IO.File]::WriteAllText(
        ...
        "@echo off`r`necho %*>>`"%PERFORMANCE_FAKE_DOCKER_LOG%`"`r`necho %* | %SystemRoot%\System32\findstr.exe /C:`"signup-smoke.js`" >nul`r`nif not errorlevel 1 exit /b %PERFORMANCE_FAKE_K6_EXIT_CODE%`r`nexit /b 0`r`n",
        $utf8WithoutBom
    )
